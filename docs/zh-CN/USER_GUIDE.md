...
3. 点击Ask获取AI的详细解答
4. **新增交互特性**：
   - **实时Markdown编辑**：右侧回答区域支持：
     - 直接修改AI生成的Markdown内容
     - 即时预览渲染效果
     - 语法高亮显示（代码块/标题/列表等）
   - **双向同步**：左侧原文修改后自动同步到AI上下文
   - **版本对比**：点击「应用修改」前显示差异对比视图
   - **历史记录**：保留最近3次问答会话（本地存储）
...
| 操作         | 快捷键        | 说明                      |
|--------------|--------------|---------------------------|
| 应用修改     | Ctrl+Enter   | 快速应用当前编辑内容       |
| 重新生成     | Ctrl+Shift+R | 基于编辑后的内容重新生成   |
| 切换模式     | Ctrl+M       | 在源码/Markdown预览间切换  |
